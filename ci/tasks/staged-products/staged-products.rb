...
products_list = File.new("#{wrkdir}/#{ENV['PCF_ENVIRONMENT']}-staged_products.json", "w")
...
puts ""
File.open("#{wrkdir}/#{ENV['PCF_ENVIRONMENT']}-staged_products.json").each do |line|
  puts line
end
products_list.close
...
file = "#{ENV['PCF_ENVIRONMENT']}-staged_products.json"
...
File.delete("#{wrkdir}/#{ENV['PCF_ENVIRONMENT']}-staged_products.json") if File.exist?("#{wrkdir}/#{ENV['PCF_ENVIRONMENT']}-staged_products.json")
