...
products_list = File.new("#{wrkdir}/#{environment}-available_products.yml", "w")
...
products = JSON.parse(`curl "#{environment_url}/api/v0/available_products" -X GET -H "Authorization: Bearer #{token}" -k -s`)
...
file = "#{environment}-available_products.yml"
...
File.delete("#{wrkdir}/#{environment}-available_products.yml") if File.exist?("#{wrkdir}/#{environment}-available_products.yml")
