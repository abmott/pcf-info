...
products_list = File.new("#{wrkdir}/#{environment}-deployed_products.yml", "w")
...
products = JSON.parse(`curl "#{environment_url}/api/v0/deployed/products" -X GET -H "Authorization: Bearer #{token}" -k -s`)
products_list.puts "#{environment.upcase} PCF Deployment Deployed Products"
products_list.puts ".................."
...
file = "#{environment}-deployed_products.yml"
...
File.delete("#{wrkdir}/#{environment}-deployed_products.yml") if File.exist?("#{wrkdir}/#{environment}-deployed_products.yml")
